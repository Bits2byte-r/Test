...
class KeyCaptureButton(QPushButton):
    ...
    def __init__(self, text="Click to assign", parent=None):
    ...
    def focusOutEvent(self, event):
        if self.is_capturing:
            self.is_capturing = False
            self.setText("Click to assign" if not self.key_str else f"Key: {self.key_str}")
...
class SimpleTab(QWidget):
    ...
    def __init__(self):
        super().__init__()
        self.points_list = []
        self.click_button = "LMB" # Default value
        self.init_ui()
    ...
    def init_ui(self):
        ...
        settings_group = QGroupBox("Simple clicker settings")
        ...
        button_layout.addWidget(QLabel("Click button:"))
        self.mouse_lmb_radio = QRadioButton("LMB")
        self.mouse_rmb_radio = QRadioButton("RMB")
        self.mouse_mmb_radio = QRadioButton("MMB")
        self.key_radio = QRadioButton("Key:")
        ...
        self.hold_checkbox = QCheckBox("Hold button (ignores interval and points)")
        settings_layout.addWidget(self.hold_checkbox)
        settings_group.setLayout(settings_layout)
        layout.addWidget(settings_group)
        points_group = QGroupBox("Click by coordinates (if empty - click cursor)")
        points_layout = QVBoxLayout()
        self.points_list_widget = QListWidget()
        points_layout.addWidget(self.points_list_widget)
        pos_layout = QHBoxLayout()
        self.add_point_btn = QPushButton("Add point (F7)")
        self.add_point_btn.setToolTip("You can also add a point by pressing F7")
        self.get_pos_btn = QPushButton("Track position")
        ...
        self.clear_points_btn = QPushButton("Clear list")
    ...
    def set_settings(self, settings):
        ...
        button = settings.get('button', 'LMB')
        if button == "LMB": self.mouse_lmb_radio.setChecked(True)
        elif button == "RMB": self.mouse_rmb_radio.setChecked(True)
        elif button == "MMB": self.mouse_mmb_radio.setChecked(True)
        else:
            self.key_radio.setChecked(True)
            self.key_capture_btn.key_str = button
            self.key_capture_btn.setText(f"Key: {button}")
        ...
        for p in points:
            if isinstance(p, list) and len(p) == 2:
                ...
                self.points_list_widget.addItem(f"Point {len(self.points_list)}: (X={p[0]}, Y={p[1]})")
    ...
    def update_button_choice(self):
        ...
        if self.mouse_lmb_radio.isChecked(): self.click_button = "LMB"
        elif self.mouse_rmb_radio.isChecked(): self.click_button = "RMB"
        elif self.mouse_mmb_radio.isChecked(): self.click_button = "MMB"
    ...
    def toggle_pos_tracking(self):
        if self._get_pos_timer.isActive():
            ...
            self.get_pos_btn.setText("Track position")
        else:
            self._get_pos_timer.start()
            self.get_pos_btn.setText("Stop tracking")
    ...
    def add_current_pos(self):
        ...
        self.points_list_widget.addItem(f"Point {len(self.points_list)}: (X={x}, Y={y})")
